...
| **caméra de visée** | AimCamera, aim, is_active, fire, réticule | fw | V2 : marqueur `AimCamera` (`forgia-player`) porté par la `FpsCamera` **et** la caméra 3P (`castle_avatar.rs`, on_enter) — `Camera::is_active` départage | `fire_weapon_minimal` (`forgia-fps`, FixedUpdate/Combat) — **seul lecteur** : le tir suit la caméra QUI REND, sinon le réticule ment | via `forgia2_hitscan.json` (origine du rayon) | * | L | int |
| **arme en main (3P)** | arme_main, socket_main_droite, MuzzleWorld, bouche, RightHand | fw+def | V2 : `EquippedWeapons.current` (arme) + `expedition_arme_main.toml` (taille/rotation/décalage, **hot-reload**) ; échelle **dérivée de l'AABB**, jamais déclarée | `arme_main.rs` accroche sous `socket_main_droite` (frame, Expédition) ; publie `MuzzleWorld` (PostUpdate, après propagation) → lueur + traceur du chemin de tir | V2 : `forgia2_expedition_arme.json` (story-717 — socket/calibrage/bouche, 5 causes distinguées) | - | L | int |
| **corps de l'avatar** | expedition_body, CorpsAvatar, stylized_male, chien_expedition, cheveux, socket_ | def+fw | `assets/genomes/roguelite/roguelite_equipment.toml` → `[expedition_body].model` (**seule** vérité ; hot-reload). Trois corps interchangeables sans code : `stylized_male_complet` (origine) · `stylized_male_cheveux` (**ACTIF 2026-08-16**, 2 os capillaires animés) · `chien_expedition` (tête sculptée, 69 os, morph `cligner`) | `equipment.rs::corps_expedition()` (on_enter) ; `avatar.rs::sys_bind_avatar_animations` — câble **TOUS** les `AnimationPlayer`, pas le premier ; `arme_main.rs` + `avatar_vfx.rs` lisent les 4 `socket_*` (des **EMPTY**, à ne jamais filtrer à l'export) | ⚠️ **AUCUN** — un capteur `forgia2_avatar.json` manque (os trouvés, morph présent, sockets) | - | L | int |
| **clé d'arme** | genome_key, vm_key, weapon_genome_key, pepin, bourrasque | fw | V2 : `WeaponType::genome_key()` (`forgia-combat/weapons.rs`) — **source unique** depuis 2026-08-15 | `forgia_viewmodel::weapon_genome_key` (délègue) ; `forgia_mode_roguelite::weapon_select::vm_key` (délègue) ; `arme_main::chemin_glb` | - | - | L | int |
| **capteur** | capteur, sensor, severity, next_step, constat, echantillon | fw | `forgia_core::constat` — `Constat<E>` type-etat (2026-08-18) : `publier()` est la SEULE sortie, `alerte()` exige `.remede()` **a la compilation**, `.echantillon(0)` degrade `ok`→`info` | 132 producteurs dans 60+ crates ; 2 migres (`anim_sensor.rs`, `contrat_mode.rs`), **38 sans verdict** | les 138 `forgia2_*.json` eux-memes ; cliquet `cargo run -p xtask -- capteur-gate` | - | L | int |
...
| **cape / mouvement secondaire** | cape, cloak, SpringBone, secondary_motion | fw+def | `forgia-secondary-motion::solver::update_spring_bones` (PostUpdate, avant `TransformSystems::Propagate`) ; `expedition_cape.toml` (hr) | `forgia-mode-expedition/src/cape.rs` accroche les 6 os `cloak_*` sous `root.001` (frame, Expédition) ; **seul consommateur actif** — la queue de Rex est débranchée depuis 2026-06-04 | `forgia2_expedition_cape.json` (os_de_chaine, solveur_a_pris) + `forgia_anim_layer.json::spring` | * | L | int |
| **retargeting d'animation** | retarget, mixamo, clip, bone orientation | def | `tools/assets/mixamo_recolte.py` (FBX→GLB, `automatic_bone_orientation=False`) → `merge_gltf_anims.py` (rotations seules, sauf `Hips`) | `assets/models/characters/stylized/stylized_male_fusil.glb` (34 clips) ; `roguelite_equipment.toml::[expedition_body.animation]` | `tools/assets/rapport_clips.json` + `verifier_corps.py` (cliquet CI) | - | L | int |
