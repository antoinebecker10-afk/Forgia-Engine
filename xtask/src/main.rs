...
// 2. plafond abaissé après le déménagement (83 163 mesurés → 84 000), puis après
//    la phase B de l'audit du même jour — 15 règles compressées, récits descendus
//    en mémoire : 42 707 o mesurés → 44 000. Toujours abaisser après un
//    dégraissage, jamais monter pour faire passer un ajout.
const CONTEXT_BUDGET_BYTES: u64 = 44_000;
...
fn dist_roguelite(args: &[String]) -> i32 {
    ...
    let readme = "FORGIA ROGUELITE — build de test\n\
        ...
        \n\
        Un souci / crash ? Le fichier forgia2_crash.json est créé à côté du jeu — joins-le à ton rapport de bug.\n";
    ...
    println!(
        "       butler push \"{}\" <utilisateur-itch>/forgia-roguelite:windows",
        out.display()
    );
    0
}
