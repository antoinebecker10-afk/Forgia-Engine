...
> ou symboles qu'elle cite ont été retrouvés dans le dépôt (capteur `forgia2_merchant.json`, fichier `boss_portal.rs`, symbole `MetaSouls`).
...
> **État d'origine (périmé, cf bandeau)** : CODE-COMPLETE + QA OK (2026-06-20) — reste validation runtime
