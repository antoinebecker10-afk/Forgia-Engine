...
**Statut** : IN_PROGRESS (incrément 1)
...
| # | Contenu | Statut |
|---|---|---|
| 1 | Les 8 correctifs du prototype portés proprement (cfg-gates wasm, web-time, tonemapping) — `cargo check` vert sur les DEUX cibles | IN_PROGRESS |
...
- [ ] `cargo check` (natif) vert, 0 warning nouveau
- [ ] `cargo check --target wasm32-unknown-unknown` vert
- [ ] `cargo clippy --workspace` : 0 warning sur fichiers touchés
- [ ] Natif inchangé : file_watcher (hot-reload) et basis-universal toujours actifs
      en natif via dépendance target-gated ; `WgpuSettingsPriority` par défaut en natif
- [ ] Tests existants verts
- [ ] Le worktree `D:\forgia-web-worktree` peut être supprimé : `build-web.ps1`
      reconstruit depuis la branche
