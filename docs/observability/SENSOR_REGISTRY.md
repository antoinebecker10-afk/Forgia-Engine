...
| `forgia2_roguelite_state.json` | T0 | forgia-mode-roguelite | `src/sensor.rs:88` | 1Hz | run_state, wave, bots_alive, victory, weapon_levels + mastery_cap (story-668), room_kind + room_budget_credits + room_density (story-669) | active |
...
| `forgia2_contrat_mode.json` | T0 | forgia-observability | `src/contrat_mode.rs:213` | 0.5Hz | **Un mode rend-il ce qu il a pris ?** Composants poses sur des entites PARTAGEES (camera, fenetre) qui survivent a la sortie du mode : Atmosphere (2 = panique wgpu, comptee MEME hors-ecran), Hdr (ecran noir), DistanceFog, AmbientLight, curseur pris hors jeu. Les trois derniers ne sont comptes que sur les cameras rendant dans la FENETRE — une camera hors-ecran (fond 3D du menu) ne lègue rien ; `cameras_hors_ecran` publie combien ont ete exemptees | active |
| `forgia2_animation.json` | T0 | forgia-anim-debug | `src/anim_sensor.rs:230` | 2Hz | **QUEL clip joue** : nom **declare** au glTF (`rifle_idle`, via `Gltf::named_animations` — jamais le label indexe `#Animation24`), `source` = fichier, poids, vitesse, seek_time · `cibles_declarees` vs `cibles_touchees` (62/0 = clip d un autre squelette) · sépare `cibles_inertes` des `cibles_mouvement_secondaire` prises par ressorts ; alerte si cheveux/cape ne sont pris par aucune couche · sépare les causes de « ça ne bouge pas » | active |
| `forgia2_matiere.json` | T0 | forgia-mode-expedition | `src/matiere.rs:295` | 1Hz | habillage des 28 aplats du Vallon : materiaux habilles/ecartes, echelle UV — alerte MATIERE_INERTE apres 12 s | active |
| `forgia2_expedition_arme.json` | T0 | forgia-mode-expedition | `src/arme_main.rs:1000` | 1Hz | arme en main 3P : socket, calibrage, bouche du canon — alerte ARME_DE_TRAVERS (autre terminal) | active |
| `forgia2_expedition_cape.json` | T0 | forgia-mode-expedition | `src/cape.rs:250` | 1Hz | cape + cheveux 3P : os trouvés, chaînes attendues/initialisées et prise réelle par le solveur d'os-ressorts — alertes CAPE_SANS_RACINE / CHAINE_TROP_COURTE / CHEVEUX_INERTES / SOLVEUR_INACTIF | active |
| `forgia2_expedition_posture.json` | T0 | forgia-mode-expedition | `src/posture.rs:240` | 1Hz | accroupi et glissade : facteur de vitesse DEMANDE au genome contre CONSTATE sur le mouvement — alerte GENE_NON_APPLIQUE quand les deux divergent | active |
| `forgia2_expedition_campements.json` | T0 | forgia-mode-expedition | `src/campements.rs:52` | 1Hz | campements du Vallon : ennemis places, abris (autre terminal) | active |
| `forgia2_expedition_carte.json` | T0 | forgia-mode-expedition | `src/capteur.rs:163,224` | 1Hz | carte du Vallon : cellules chargees, verdict de streaming — alerte STREAMING INUTILE (autre terminal) | active |
| `forgia2_expedition_visee.json` | T0 | forgia-mode-expedition | `src/visee.rs:994` | 1Hz | visee 3P : facteur, champ de vision, pose de la tenue (autre terminal) | active |
...
| `forgia2_castle_lightmaps.json` | T0 | forgia-game | `src/castle_lightmaps.rs:52` | 1Hz | Hall de Forgia : lumière cuite du pack — pièces connues de la table, pièces éclairées, atlas chargés, sens vertical appliqué ; alert critical si la table est vide ou si aucune pièce n'est éclairée passé le délai de streaming | active |
| `forgia2_castle_envmap.json` | T0 | forgia-game | `src/castle_envmap.rs:47` | 1Hz | Hall de Forgia : éclairage par image — cubemap chargée/réinterprétée, caméras porteuses, intensité ; alert critical si la cubemap est illisible ou jamais chargée, warn si aucune caméra ne la porte | active |
...
| `forgia2_arena_test.json` | T0 | forgia-game | `src/arena_test.rs:36` | 1Hz | banc de blockout : pièces posées (pavés/disques/rampes/murs), rampe la plus raide, marche de traversée la plus haute, forme et emprise de la carte, marge entre le sol et l'enceinte, part de SA VOIE dominée par la position de force la plus forte, hauteur requise à mi-portée pour la contester vs structure la plus haute réellement posée, altitude joueur, dérive des metrics déclarées vs `player_movement.toml` ; alert critical si le genome est illisible ou l'arène vide, warn si le sol n'atteint pas les sommets de l'hexagone (chute hors-map dans les coins), si une marche dépasse la hauteur de saut (route infranchissable), si un point haut domine plus de 85 % de la fosse (point haut acquis au lieu d'être contesté) ou si les metrics sont périmées — story-667 | active |
