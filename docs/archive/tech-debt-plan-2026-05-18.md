> **ARCHIVÉ 2026-05-19** — obsolète à 80 %. Phases 1, 2, 3, 4, 6, B1 livrées
> silencieusement par sessions ultérieures (cf `docs/ROADMAP_CURRENT.md` V4).
> Restent uniquement : Phase 5 (VFX colors per-arme) + Phases 7-8 (extraction
> crates `forgia-weapon-hitscan`/`viewmodel`, bloquées par recovery WIP fire
> system 2026-05-17). Conservé pour traçabilité historique.
...
| **5** | VFX colors per-arme (audit + dispatch) | 1h | Medium | quick | ✅ DONE 2026-05-19 (muzzle/impact tint + PointLight bref + sniper smoke trail) |
