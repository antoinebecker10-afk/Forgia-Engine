...
| **Bugs playtest 2026-08-09 (cycles ouvrir/fermer l'arène)** | 3 bugs rapportés par le user, triés capteurs+log le soir même. **(a) Salle 1 ≠ chapitre** : sur 7 lancements, 4× `forge_sanctum` / 2× `hauts_paturages` — le stage est tiré au `run_seed` sans lire `SelectedChapter`, alors que le menu promet l'arène du chapitre (backdrop `forge_crepuscule`). **(b) Munitions non refill au start** : `forgia2_combat.json` à 8 s de run = AssaultRifle 10/30, Pépin 0/12 (36/84) — l'état survit au cycle quitter→menu→relancer, pas de reset `StartRunEvent` sur le chemin abandon. **(c) Mobs « me sautent dessus » + confinés** : anneaux d'apparition 12/25 m centrés joueur (grunt 9 m/s = contact en ~2 s, anneau DANS les pièces autorées) + bots sans navmesh limités à `alert_radius` 25 m dans une arène de 80-90 m + 0 abri (`stage_layout` error) — remède structurel déjà décidé : vleue_navigator depuis `ArenaGeometry`, story dédiée (cf audit arènes 2026-08-06). ⚠️ (a) et (b) touchent l'arbre chaud de l'autre terminal (`feat/arena-authored-shell-625`, `ammo.rs` dans son diff) — coordonner avant fix. |
...
- **Dette technique** :
  ...
  - **Bevy 0.19** : migration devenue **possible** (2026-08-05 : `bevy_rapier3d` 0.35.0 cible ^0.19 depuis le 12/07, hanabi/leafwing/scripting ✅) mais **volontairement différée** — cœur `rapier3d` encore en **alpha** + `bevy_water` dormant + cycle validation/courbe de puissance en cours. Fenêtre & conditions : [`migration/bevy-019-blockers.md`](migration/bevy-019-blockers.md) § Décision de fenêtre. **Ne rien bloquer dessus** ; plan B assumé = shipper sur 0.18.1.
