...
use crate::manifest::{EnveloppesCollision, ExpeditionManifest};

const MANIFESTE_GAMEPLAY: &str = "assets/models/environment/expedition/expedition_vallon.json";
/// Les enveloppes convexes de collision, produites par
/// `tools/blender/expedition/15_colliders.py`. Absent ⇒ repli sur les
/// cylindres du manifeste, dit au journal.
const MANIFESTE_ENVELOPPES: &str =
    "assets/models/environment/expedition/vallon_colliders_formes.json";
...
pub struct ActiveExpedition {
    ...
    /// La MEME grandeur, ventilee. Elle vit ici et non au manifeste parce
    /// qu'un total qui tient pendant qu'une famille se vide ne se verrait
    /// pas — et parce que lire la ventilation dans la DECLARATION pendant
    /// que le total compte le POSE fait diverger deux grandeurs d'une meme
    /// source, ce qui est la classe de defaut n°1 de ce projet.
    pub colliders_par_famille: std::collections::HashMap<String, u32>,
...
fn setup_expedition(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    // Les ENVELOPPES CONVEXES d'abord. Mesuré à la cuisson : le cylindre
    // englobait en moyenne 2,95 fois trop de volume — jusqu'à 5,64 fois sur
    // un campement et 3,64 sur un bouchon de ceinture. Sur un abri, cette
    // marge est du gameplay volé : on se cogne à un mètre du rocher, et le
    // tir qu'on croit rasant part dans du vide solide.
    let enveloppes = forgia_core::def_io::read_def_str(MANIFESTE_ENVELOPPES)
        .map_err(|e| e.to_string())
        .and_then(|s| EnveloppesCollision::parse(&s).map_err(|e| e.to_string()));

    let mut poses = 0u32;
    let mut refusees = 0u32;
    let mut par_famille: std::collections::HashMap<String, u32> =
        std::collections::HashMap::new();
    match &enveloppes {
        Ok(env) => {
            for (famille, pos, rot, points) in env.colliders_bevy() {
                // `convex_hull` rend `None` sur une forme dégénérée (points
                // coplanaires). On le COMPTE : un solide qui disparaîtrait
                // ici serait du décor traversable, donc pire que trop gros.
                let Some(collider) = Collider::convex_hull(&points) else {
                    refusees += 1;
                    continue;
                };
                commands.spawn((
                ...
                *par_famille.entry(famille.to_string()).or_insert(0) += 1;
                poses += 1;
            }
            info!(
                "[expedition] {poses} enveloppes convexes posées ({refusees} refusées), \
                 {} formes partagées",
                env.formes.len()
            );
        }
        Err(e) => {
            // REPLI DIT. Le manifeste porte encore ses cylindres : mieux vaut
            // une carte trop solide qu'une carte traversable.
            warn!(
                "[expedition] enveloppes illisibles ({e}) — repli sur les \
                 cylindres du manifeste, moins précis d'un facteur ~3"
            );
    ...
    commands.insert_resource(ActiveExpedition {
        ...
        colliders_par_famille: par_famille,
        spawn_pending: true,
    });
}
...
fn place_player_when_ready(
    ...
    let Ok((mut xf, mut player)) = q_player.single_mut() else {
    ...
    let direction = Vec3::new(regard.x - depart.x, 0.0, regard.y - depart.z);
    let yaw = yaw_vers(direction);
    *xf = Transform::from_translation(depart).with_rotation(Quat::from_rotation_y(yaw));
    // Le contrôleur déplace selon `Player.yaw`, tandis que la caméra orbitale
    // suit la rotation du Transform. N'en initialiser qu'un crée deux « avant »
    // opposés : le personnage regarde le chemin mais W l'envoie dans la forêt.
    player.yaw = yaw;
...
/// Yaw qui aligne l'avant conventionnel du joueur (`-Z`) sur `direction`.
fn yaw_vers(direction: Vec3) -> f32 {
...
fn stream_expedition_cells(
    ...
    for (entity, cell) in &q_loaded {
        ...
        if cells::decide(cells::horizontal_distance(pos, spec), true, active.radii)
    ...
    for spec in &active.cells.cells {
        ...
        if cells::decide(cells::horizontal_distance(pos, spec), false, active.radii)
            == CellAction::Charger
        {
...
mod tests {
    ...
    #[test]
    fn le_yaw_du_spawn_envoie_l_avant_du_joueur_sur_le_chemin() {
        ...
        let depart = m.spawn_player_origin();
        let regard = m.regard_bevy();
        let direction = Vec3::new(regard.x - depart.x, 0.0, regard.y - depart.z).normalize();
        let yaw = yaw_vers(direction);
        let avant = Quat::from_rotation_y(yaw) * Vec3::NEG_Z;
        assert!(
            avant.dot(direction) > 0.9999,
            "avant {avant:?}, chemin {direction:?}"
        );
    }
    ...
    fn le_rayon_de_chargement_couvre_ce_qu_on_voit_sans_charger_toute_la_carte() {
        ...
        assert!(
            RAYON_CHARGEMENT_M > 100.0,
            "le decor apparaitrait devant le joueur"
        );
        assert!(
