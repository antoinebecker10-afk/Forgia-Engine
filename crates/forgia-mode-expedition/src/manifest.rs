...
/// Une enveloppe convexe, en LOCAL Blender et à l'échelle du maillage.
///
/// Elle est PARTAGÉE : une forme par pièce du kit, réutilisée par toutes ses
/// instances. C'est ce partage qui rend la précision gratuite à la cuisson.
#[derive(Debug, Clone, Deserialize)]
pub struct FormeCollision {
    pub famille: String,
    pub points: Vec<[f32; 3]>,
}

/// Une instance : quelle forme, où, tournée comment, à quelle échelle.
///
/// # Pourquoi la rotation arrive DÉJÀ convertie
///
/// Un point se convertit composante par composante ; une rotation NON — il
/// faut un changement de base `M · R · Mᵀ`. Faire ce calcul ici obligerait à
/// l'écrire une deuxième fois, et une convention écrite deux fois finit
/// toujours par diverger sur un site. Elle est donc calculée une seule fois,
/// dans `15_colliders.py`, qui la vérifie à chaque export : tourner puis
/// convertir doit égaler convertir puis tourner, sinon il lève.
#[derive(Debug, Clone, Deserialize)]
pub struct InstanceCollision {
    pub forme: String,
    pub famille: String,
    /// Repère Blender — converti ici.
    pub pos: [f32; 3],
    /// Repère **Bevy**, en `(w, x, y, z)`.
    pub rot: [f32; 4],
    /// Échelle Blender, à permuter comme les axes.
    pub ech: [f32; 3],
}

/// Les enveloppes de collision du Vallon, formes et poses.
#[derive(Debug, Clone, Deserialize)]
pub struct EnveloppesCollision {
    pub formes: std::collections::HashMap<String, FormeCollision>,
    pub instances: Vec<InstanceCollision>,
}

impl EnveloppesCollision {
    /// Lit le fichier d'enveloppes, et **refuse** un fichier vide.
    ///
    /// Un fichier vide accepté donnerait une carte entièrement traversable
    /// sans une ligne de journal — le pire échange possible.
    pub fn parse(contenu: &str) -> Result<Self, ExpeditionManifestError> {
        let e: Self = serde_json::from_str(contenu)
            .map_err(|err| ExpeditionManifestError::Json(err.to_string()))?;
        if e.formes.is_empty() || e.instances.is_empty() {
            return Err(ExpeditionManifestError::Vide);
        }
        Ok(e)
    }

    /// Rend, pour chaque instance : famille, translation, rotation et les
    /// points de l'enveloppe **déjà en repère Bevy et à l'échelle**.
    ///
    /// L'échelle se permute comme les axes : `Bevy(x, y, z) = Blender(x, z, −y)`
    /// donne `ech_bevy = (ex, ez, ey)`. Le signe, lui, est porté par le point.
    pub fn colliders_bevy(&self) -> impl Iterator<Item = (&str, Vec3, Quat, Vec<Vec3>)> + '_ {
        self.instances.iter().filter_map(move |i| {
            let forme = self.formes.get(&i.forme)?;
            let ech = Vec3::new(i.ech[0], i.ech[2], i.ech[1]);
            let points: Vec<Vec3> = forme
                .points
                .iter()
                .map(|p| blender_to_bevy(*p) * ech)
                .collect();
            let rot = Quat::from_xyzw(i.rot[1], i.rot[2], i.rot[3], i.rot[0]);
            Some((i.famille.as_str(), blender_to_bevy(i.pos), rot, points))
        })
    }

    /// Combien d'instances, toutes familles confondues.
    #[must_use]
