...
pub struct CastleLighting {
    ...
    /// Refiltrer la cubemap d'ambiance a CHAQUE image ?
    ///
    /// 🚨 MESURE le 2026-08-21 : `false` rend **2,42 ms par image**, soit 9,1 % du
    /// temps de frame du Hall. Bevy 0.18 n'a AUCUN garde-fou « deja genere » —
    /// `extract_generated_environment_map_entities` (bevy_pbr light_probe/generate.rs)
    /// extrait chaque frame toute entite portant `GeneratedEnvironmentMapLight`, et
    /// le filtrage GPU complet (copie du mip 0, chaine de mips, deux convolutions)
    /// se rejoue integralement. Pour une cubemap qui ne change JAMAIS.
    ///
    /// A `false` on retire le generateur une fois le filtrage fait : le
    /// `EnvironmentMapLight` que Bevy a pose garde les cartes filtrees, et
    /// l'intensite reste reglable a chaud (c'est un multiplicateur de shader).
    /// A `true`, comportement d'origine — utile si un jour l'ambiance devient
    /// dynamique (cycle jour/nuit, changement de zone).
    pub env_refiltrer_chaque_image: bool,
    ...
    ///
    /// 🚨 Le 0,05 longtemps annoncé ici n'avait AUCUNE source. Mesuré le 2026-08-21
    /// sur les 46 137 344 texels des 11 atlas (luminance BT.709) : médiane globale
    /// 0,0200, **médiane des texels allumés 0,0259**, moyenne 0,0291, max 98,4.
    /// À gain 1 (le défaut de Bevy) la lumière cuite ne pèse rien — ce facteur fait
    /// le pont entre les deux unités.
    ///
    /// La valeur DÉPEND du mode (`bake_includes_direct`) :
    /// - cuisson qui REMPLACE le direct : 400 / 0,0259 ≈ 15 500 (elle tient lieu
    ///   d'ambiante) ;
    /// - cuisson qui S'AJOUTE (le cas de ces atlas) : bien plus bas, sinon on
    ///   empile un rebond sur une ambiante qui en tient déjà lieu.
    pub lightmaps_exposure: f32,
    /// La cuisson contient-elle la lumiere DIRECTE, ou seulement le rebond ?
    ///
    /// 🚨 MESURE le 2026-08-21 sur les 11 atlas : les toits en plein soleil ont une
    /// luminance mediane de **0,0099**, MOINS que les murs interieurs (0,0200) et a
    /// peine plus que les plafonds (0,0088). Une cuisson contenant le soleil ferait
    /// des toits les surfaces les plus brillantes de l'atlas — c'est l'inverse.
    /// Le nom des sources le confirme (`Lightmap-N_comp_light.exr`), et le createur
    /// s'appuyait sur des shadowmasks pour son direct.
    ///
    /// **Ces atlas sont donc du rebond SEUL.** D'ou ce drapeau : a `false`, la
    /// lumiere cuite s'AJOUTE a l'ambiante, au soleil et au remplissage au lieu de
    /// les remplacer. C'est ce qui evite la couture — 14 % des maillages du Hall
    /// n'ont pas de lightmap, et les retirer du direct les laissait seuls avec une
    /// ambiante froide au milieu de voisins cuits et chauds.
    ///
    /// A `true`, on retrouve le comportement d'une cuisson COMPLETE (direct inclus),
    /// qui remplace le temps reel pour ne pas compter deux fois.
    pub bake_includes_direct: bool,

    /// Occlusion de contact en espace écran.
    ///
    /// 🚨 Elle n'agit QUE sur la lumière **indirecte** (ambiante + éclairage par
    /// image) — jamais sur le soleil ni sur les 56 lumières du créateur. C'est
    /// précisément ce qu'il faut ici : le Hall n'a plus aucun rebond depuis que
    /// la lumière cuite est coupée, et son ambiante de 400 se pose à plat sur
    /// tout. Le SSAO recreuse les coins et rend le contact au sol.
    ///
    /// Le jumeau vit dans `forgia-mode-roguelite::render_quality`, où il est
    /// coupé pour une raison qui ne vaut PAS ici : il blanchit l'arène en se
    /// combinant au post-traitement toon (constaté en jeu le 2026-06-30). Le
    /// Hall n'a pas ce post-traitement.
    pub ssao_enabled: bool,
    /// 0 Low · 1 Medium · 2 High · 3 Ultra.
    pub ssao_quality: u8,
    /// Épaisseur estimée des objets, en mètres (0,01 à 2,0).
...
impl Default for CastleLighting {
    fn default() -> Self {
        Self {
            ...
            env_enabled: true,
            env_intensity: 900.0,
            env_refiltrer_chaque_image: false,
            lightmaps_enabled: true,
            lightmaps_flip_v: false,
            // 400 (l'ambiante remplacée) / 0,0259 (la médiane MESURÉE sur les
            // 46 M texels des 11 atlas, 2026-08-21) = 15 455. Le 0,05 précédent
            // n'avait aucune source : la lumière cuite pesait moitié moins que
            // ce qu'elle remplaçait, d'où le « trop sombre » qui l'a fait couper.
            lightmaps_exposure: 15_500.0,
            bake_includes_direct: false,
            // false : un effet de rendu jamais vu en jeu ne s'allume pas tout
            // seul. Le genome decide, et le capteur dit s'il est pose.
            ssao_enabled: false,
            ssao_quality: 2,
            ssao_thickness: 0.25,
        }
...
impl CastleLighting {
    /// La lumiere cuite REMPLACE-t-elle le direct temps reel ?
    ///
    /// Deux conditions, et il faut les DEUX : que la cuisson soit allumee, et
    /// qu'elle contienne effectivement le direct. Les confondre — ce que faisait
    /// `!lightmaps_enabled` seul — retire l'ambiante et le soleil sur la foi d'une
    /// hypothese jamais verifiee sur les atlas.
    fn remplace_le_direct(&self) -> bool {
        self.lightmaps_enabled && self.bake_includes_direct
    }
    ...
    pub fn ambient(&self) -> AmbientLight {
        AmbientLight {
            ...
            // Le direct ne se retire QUE si la cuisson le contient — voir
            // `bake_includes_direct`. Sur ces atlas-ci (rebond seul), il reste.
            affects_lightmapped_meshes: !self.remplace_le_direct(),
        }
    }

    fn emissive(&self) -> LinearRgba {
        let [r, g, b] = self.light_color;
        let luminance = self.emissive_luminance.max(0.0);
        // 🚨 L'ALPHA N'EST PAS UNE COULEUR — ET IL SE FAISAIT MULTIPLIER (2026-08-21).
        //
        // `LinearRgba::rgb(..) * 9000.0` mettait aussi l'alpha à 9000 : la macro
        // `impl_componentwise_vector_space!(LinearRgba, [red, green, blue, alpha])`
        // (bevy_color 0.18) multiplie TOUS les champs listés, alpha compris.
        //
        // Or le shader s'en sert comme d'un facteur d'interpolation dans [0,1] —
        // `bevy_pbr/src/render/pbr_functions.wgsl:722` :
        //     emissive_light *= mix(1.0, view.exposure, emissive.a);
        // Avec `a = 9000` et l'exposition par défaut de Bevy (aucune `Exposure`
        // n'est posée dans Forgia, donc `EV100_BLENDER = 9.7` → `exp2(-9.7)/1.2 =
        // 1,0019e-3`), ce `mix` vaut **−8990**. L'émissif partait donc négatif, et
        // le tonemapping l'écrêtait à zéro (`max(in.rgb, vec3(0.0))`).
        //
        // Conséquence : les 762 gouttes de flamme rendaient du NOIR PUR — leur
        // `base_color` est déjà `Color::BLACK`, il ne restait rien à voir. Personne
        // ne l'avait vu parce que le défaut est silencieux : pas de warning, pas
        // d'erreur, juste une mèche éteinte au milieu d'un décor éclairé.
        //
        // `a = 1.0` est le bon choix : `emissive_luminance` est une luminance
        // physique en cd/m² (c'est ce que le génome déclare), donc elle DOIT passer
        // par l'exposition de la caméra — ce que `mix(1.0, exposure, 1.0)` fait.
        LinearRgba::new(r * luminance, g * luminance, b * luminance, 1.0)
    }
    ...
    pub fn parse_toml(content: &str) -> Self {
        ...
        let ssao = parsed.ssao.unwrap_or_default();
        Self {
            ...
            env_refiltrer_chaque_image: environment
                .refiltrer_chaque_image
                .unwrap_or(base.env_refiltrer_chaque_image),
            ...
            bake_includes_direct: lightmaps
                .bake_includes_direct
                .unwrap_or(base.bake_includes_direct),
            ssao_enabled: ssao.enabled.unwrap_or(base.ssao_enabled),
            // Borné : une qualité hors table donnerait un `match` avec un bras
            // par défaut silencieux, et personne ne saurait que la valeur du
            // fichier n'a pas été prise.
            ssao_quality: ssao.quality.unwrap_or(base.ssao_quality).min(3),
            ssao_thickness: ssao
                .thickness
                .unwrap_or(base.ssao_thickness)
                .clamp(0.01, 2.0),
        }
    }
...
struct LightmapsToml {
    ...
    flip_v: Option<bool>,
    exposure: Option<f32>,
    bake_includes_direct: Option<bool>,
}
...
struct EnvironmentToml {
    ...
    refiltrer_chaque_image: Option<bool>,
}
...
#[derive(Component)]
pub(crate) struct CreatorLight;

#[derive(Resource, Default)]
pub(crate) struct CreatorLightCount(pub(crate) u32);
...
fn sys_apply_sun(
    ...
    // pièces réimportées.
    let affects_baked = !tuning.remplace_le_direct();
...
fn severity_for_flames(
    ...
    // 🚨 « ok » sur du vide est un mensonge (controle-de-la-sortie §1, et c'est
    // la raison d'etre de `forgia_core::constat`). Hors du Hall il n'y a rien a
    // mesurer : c'est `info`. Flammes eteintes, c'est un choix declare, donc
    // aussi `info` — mais surtout PAS un feu vert : un capteur qui verdit quand
    // on eteint ce qu'il surveille ne surveille plus rien.
    if !in_hub {
        return ("info", "hors du Hall — aucune flamme a mesurer");
    }
    if !enabled {
        return ("info", "flammes desactivees dans castle_hub_lighting.toml [flames]");
    }
...
mod tests {
    ...
    fn missing_flames_in_hub_is_critical() {
        ...
        // 🚨 Ces deux cas rendaient « ok » : sortir du Hall, ou eteindre les
        // flammes, suffisait a verdir le capteur. Un zero mesure n'est pas vert,
        // il est aveugle — et le test qui gravait l'inverse etait le vrai defaut.
        assert_eq!(severity_for_flames(false, true, 0, 0).0, "info");
        assert_eq!(severity_for_flames(true, false, 0, 0).0, "info");
    }
}
