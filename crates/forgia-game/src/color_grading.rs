...
pub struct ModeGrade {
    ...
    pub contrast: f32,
    pub saturation: f32,
    /// Décalage linéaire du bas de la plage (le *o* de la formule ASC CDL).
    /// Négatif = noirs plus profonds ; positif = ombres délavées.
    ///
    /// 🚨 Ce n'est PAS la « teinte bleue des ombres » du créateur du pack.
    /// `ColorGrading::shadows` de Bevy ne porte que des **scalaires**
    /// (`saturation`, `contrast`, `gamma`, `gain`, `lift`) : aucun canal RGB, donc
    /// son `(0.767, 0.808, 1.0)` n'y rentre pas. Cette teinte est déjà obtenue
    /// autrement, et plus physiquement : l'ambiante du Hall est froide
    /// (`[0.72, 0.80, 0.92]` dans `castle_hub_lighting.toml`) et c'est elle qui
    /// éclaire ce que rien ne frappe — c'est-à-dire les ombres.
    pub shadows_lift: f32,
    /// Ajustement non linéaire du bas de la plage (le *n* de la formule ASC CDL).
    /// > 0 creuse les ombres sans toucher aux hautes lumières.
    pub shadows_gamma: f32,
}
...
impl ModeGrade {
    ...
    pub fn to_color_grading(self) -> ColorGrading {
        ...
        // Les ombres, séparément des midtones : c'est la seule façon de creuser
        // le bas de la plage sans écraser le reste. Sans rebond ni occlusion, un
        // intérieur n'a pas de vrais noirs — tout remonte au plancher de
        // l'ambiante. Ces deux curseurs naissent NEUTRES : ils ne changent rien
        // tant qu'on ne les a pas jugés en jeu, comme le reste du fichier.
        cg.shadows.lift = (cg.shadows.lift + self.shadows_lift).clamp(-1.0, 1.0);
        cg.shadows.gamma = (cg.shadows.gamma + self.shadows_gamma).clamp(0.0, 4.0);
        cg
    }
}
...
struct ModeGradeToml {
    ...
    shadows_lift: Option<f32>,
    shadows_gamma: Option<f32>,
}
...
fn merge(base: ModeGrade, t: &Option<ModeGradeToml>) -> ModeGrade {
    match t {
        ...
        Some(t) => ModeGrade {
            ...
            shadows_lift: t.shadows_lift.unwrap_or(base.shadows_lift),
            shadows_gamma: t.shadows_gamma.unwrap_or(base.shadows_gamma),
        },
    }
}
...
fn sys_apply(
    ...
    q_cam_all: Query<
        ...
        (
            With<Camera3d>,
            Without<forgia_core::prelude::UiStudioCamera>,
        ),
    >,
    q_cam_new: Query<
        Entity,
    ...
    let ambiance_grade = match (amb_cfg.as_deref(), current_amb.as_deref()) {
        (Some(c), Some(cur)) if mode == GameMode::Roguelite => {
            c.ambiance(&cur.id).grade.map(|g| ModeGrade {
                ...
                // Les ambiances de l'univers Roguelite ne décrivent pas la
                // profondeur des ombres : leur structure ne porte pas ces
                // champs. Neutres, donc — et énumérés plutôt que remplis par
                // `..NEUTRAL`, pour que le compilateur continue de nommer ce
                // site le jour où `ModeGrade` gagne un champ de plus.
                shadows_lift: 0.0,
                shadows_gamma: 0.0,
            })
        }
