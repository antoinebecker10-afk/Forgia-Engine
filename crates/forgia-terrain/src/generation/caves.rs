...
pub fn carve_cave_worms(
    ...
    if base_worms == 0 { return; }

    if next_f32(&mut rng) > cave_prob { return; }

    for layer in 0..params.layers {
        let layer_frac = layer as f32 / params.layers as f32;
        let y_center = params.min_height + (params.max_height - params.min_height) * (0.3 + layer_frac * 0.5);
        ...
        for _ in 0..base_worms.max(1) {
            ...
            for step in 0..steps {
                if wy < sea_level + 2.0 || wy > params.max_height { break; }

                let radius = params.worm_radius * (0.8 + next_f32(&mut rng) * 0.4);
                carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, wx, wy, wz, radius);

                if step % 8 == 4 && next_f32(&mut rng) < params.chamber_prob {
                    let cr = params.chamber_radius_min
                        + next_f32(&mut rng) * (params.chamber_radius_max - params.chamber_radius_min);
...
pub fn carve_village_caves(
    ...
    for (idx, village) in village_network.villages.iter().enumerate() {
        ...
        let village_seed = u64::from(config.seed
        ...
        // 1. ENTRANCE
        carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, entrance_x, surface_y - 1.0, entrance_z, 5.0);
        carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, entrance_x, surface_y + 1.5, entrance_z, 4.0);
        ...
        for step in 0..=SHAFT_STEPS {
            ...
            carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, chamber_x, sy, chamber_z, 3.5);
        }

        // 4. MAIN CHAMBER
        carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, chamber_x, chamber_y, chamber_z, 18.0);
        carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, chamber_x, chamber_y + 6.0, chamber_z, 12.0);
        carve_sphere(chunk, config, origin, pad_x, pad_y, pad_z, chamber_x, chamber_y - 3.0, chamber_z, 14.0);

        // 5. BRANCH TUNNELS
        for branch in 0..4 {
            let branch_angle = (branch as f32) * std::f32::consts::FRAC_PI_2
                + entrance_angle * 0.3;
...
mod tests {
    ...
    fn cave_threshold_mountain_most_frequent() {
        assert!(cave_threshold_for_biome(BiomeType::Mountain) < cave_threshold_for_biome(BiomeType::Plains));
        assert!(cave_threshold_for_biome(BiomeType::Volcanic) < cave_threshold_for_biome(BiomeType::Desert));
    }
    ...
    fn cave_probability_ordering_matches_lore() {
        ...
        assert!(volcanic > mountain, "Volcanic should beat Mountain in cave freq");
    ...
    fn cave_probability_in_unit_range() {
        for b in [
            ...
            assert!((0.0..=1.0).contains(&p), "{b:?} cave_probability {p} outside [0,1]");
        }
    }
