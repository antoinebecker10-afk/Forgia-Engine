...
impl BiomeMap {
    ...
    pub fn generate_with_intent(
        ...
        if let Some(gc) = gen_config {
            if let BiomeMode::Single { ref biome } = gc.biome_mode {
                ...
                info!(
                    "BiomeMap: Single biome mode — {} over {:.0}m²",
                    biome,
            ...
            {
                return Self::generate_directional(
                    config, gc, center, north_west, north_east, south_east, south_west,
                );
            }
        ...
        let hex_spacing = gen_config
            .map(|gc| gc.biome_cell_size.clamp(64.0, 512.0))
            .unwrap_or(192.0);
        let mut weights = gen_config
            .map(|gc| gc.biome_weights.clone())
            .unwrap_or_default();
        ...
        while z < map_size {
            ...
            while x < map_size {
                ...
                seeds.push(BiomeSeed {
                    position: Vec2::new(jx, jz),
                    biome,
                });
                x += hex_spacing;
            }
        ...
        info!(
            "BiomeMap: generated {} Voronoi seeds (cell={:.0}m) over {:.0}m²",
            seeds.len(),
            hex_spacing,
            config.map_size * config.map_size
        );
    ...
    fn generate_altitude(config: &TerrainConfig, gen_config: &MapGenConfig) -> Self {
        ...
        while z < map_size {
            ...
            while x < map_size {
                ...
                let h_raw = perlin.get([
                    (f64::from(x)
                        + warp
                            * perlin.get([f64::from(x) * freq + 5.2, f64::from(z) * freq + 1.3]))
                        * freq,
                    (f64::from(z)
                        + warp
                            * perlin.get([f64::from(x) * freq + 9.1, f64::from(z) * freq + 4.7]))
                        * freq,
                ]);
        ...
        info!(
            "BiomeMap: Altitude mode — {} seeds over {:.0}m²",
            seeds.len(),
            map_size * map_size
        );
        Self { seeds }
        ...
        while z < map_size {
            ...
            while x < map_size {
                ...
                let biome = if dist_frac < center_radius_frac {
                    ...
                    let at_w = in_transition(angle, std::f32::consts::PI)
                        || in_transition(angle, -std::f32::consts::PI);
                    if at_n && dist_frac > 0.3 {
                        transitions[0].2
                    } else if at_e && dist_frac > 0.3 {
                        transitions[1].2
                    } else if at_s && dist_frac > 0.3 {
                        transitions[2].2
                    } else if at_w && dist_frac > 0.3 {
                        transitions[3].2
    ...
    fn generate_landmark_voronoi(
        config: &TerrainConfig,
        _gen_config: &MapGenConfig,
        intent: &WorldMapIntent,
    ) -> Self {
        let map_size = config.map_size;
        ...
        for lm in &intent.landmarks {
            ...
            let anchor_biome = lm
                .required_biome
                .unwrap_or_else(|| fallback_biome_by_quadrant(lm.x, lm.z));
        ...
        const CURATED_SEEDS: &[(BiomeType, f32, f32)] = &[
            (BiomeType::Mountain, 0.12, 0.12),
            (BiomeType::Mountain, 0.25, 0.10),
            (BiomeType::Forest, 0.22, 0.28),
            (BiomeType::Forest, 0.10, 0.28),
            (BiomeType::Mountain, 0.88, 0.10),
            (BiomeType::Mountain, 0.75, 0.08),
            (BiomeType::Forest, 0.85, 0.45),
            (BiomeType::Forest, 0.75, 0.58),
            (BiomeType::Forest, 0.90, 0.60),
            (BiomeType::Plains, 0.40, 0.50),
            (BiomeType::Plains, 0.60, 0.50),
            (BiomeType::Plains, 0.50, 0.40),
            (BiomeType::Plains, 0.50, 0.62),
            (BiomeType::Savanna, 0.12, 0.75),
            (BiomeType::Savanna, 0.30, 0.88),
            (BiomeType::Savanna, 0.25, 0.70),
            (BiomeType::Volcanic, 0.90, 0.90),
            (BiomeType::Volcanic, 0.75, 0.88),
            (BiomeType::Swamp, 0.55, 0.92),
            (BiomeType::Swamp, 0.65, 0.95),
            (BiomeType::Canyon, 0.72, 0.75),
        ];
        for (biome, nx, nz) in CURATED_SEEDS {
            seeds.push(BiomeSeed {
                position: Vec2::new(nx * map_size, nz * map_size),
                biome: *biome,
            });
        }
    ...
    pub fn biome_blend_at(&self, x: f32, z: f32) -> (BiomeType, BiomeType, f32) {
        ...
        match blend.count {
            ...
            _ => {
                let w1 = blend.biomes[1].1;
                (blend.biomes[0].0, blend.biomes[1].0, w1)
            }
        }
    }
    ...
    pub fn biome_weights_at(
        ...
        let mut nearest: [(f32, BiomeType); MAX_BLEND_BIOMES] =
            [(f32::MAX, BiomeType::Plains); MAX_BLEND_BIOMES];
        for seed in &self.seeds {
            let d = pos.distance_squared(seed.position);
        ...
        let mut result = BiomeBlend {
            biomes: [(BiomeType::Plains, 0.0); MAX_BLEND_BIOMES],
            count,
        };
        if weight_sum > 0.001 {
            for i in 0..count {
                result.biomes[i] = (nearest[i].1, weights[i] / weight_sum);
            }
        } else if count > 0 {
...
fn select_biome_weighted(weights: &BiomeWeights, rng: &mut impl FnMut() -> f32) -> BiomeType {
    ...
    let mut accum = 0.0;
    let biomes = [
        (BiomeType::Plains, weights.plains),
        (BiomeType::Forest, weights.forest),
        (BiomeType::Desert, weights.desert),
        (BiomeType::Mountain, weights.mountain),
        (BiomeType::Swamp, weights.swamp),
        (BiomeType::Tundra, weights.tundra),
        (BiomeType::Savanna, weights.savanna),
        (BiomeType::Jungle, weights.jungle),
        (BiomeType::Volcanic, weights.volcanic),
        (BiomeType::Canyon, weights.canyon),
    ];
...
pub fn setup_biome_materials(
    ...
    let materials = [
        make(BiomeType::Plains),
        make(BiomeType::Forest),
        make(BiomeType::Desert),
        make(BiomeType::Mountain),
        make(BiomeType::Swamp),
        make(BiomeType::Tundra),
        make(BiomeType::Savanna),
        make(BiomeType::Jungle),
        make(BiomeType::Volcanic),
...
mod tests {
    ...
    fn from_id_covers_all_ten_biomes() {
        use BiomeType::*;
        let expected = [
            Plains, Forest, Desert, Mountain, Swamp, Tundra, Savanna, Jungle, Volcanic, Canyon,
        ];
        for (id, expected_biome) in expected.iter().enumerate() {
            let got = BiomeType::from_id(id as u8);
            assert_eq!(
                std::mem::discriminant(&got),
                std::mem::discriminant(expected_biome)
            );
        }
    }
    ...
    fn fallback_biome_center_is_plains() {
        assert!(matches!(
            fallback_biome_by_quadrant(0.5, 0.5),
            BiomeType::Plains
        ));
    }
}
