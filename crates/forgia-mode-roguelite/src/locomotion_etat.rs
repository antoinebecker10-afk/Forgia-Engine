...
pub fn choisir(e: &EtatLocomotion, s: &Seuils) -> Clip {
    ...
    let lateral = if absurde(e.lateral_ms) { 0.0 } else { e.lateral_ms };
    ...
    if !e.au_sol && (e.depuis_decollage_s >= s.air_min_s || e.vitesse_verticale_ms > s.montee_min_ms)
    {
    ...
    if e.depuis_atterrissage_s < s.atterrissage_s {
        return Atterrissage;
    }
...
mod tests {
    ...
    fn chaque_clip_a_un_repli_qui_finit_sur_idle() {
        ...
        for c in [
            Marche, Course, MarcheArriere, PasDeCote, Visee, Tir, Saut, Chute,
            Atterrissage, AccroupiIdle, AccroupiMarche, AccroupiArriere, Glissade, Virage,
        ] {
            let r = c.replis();
