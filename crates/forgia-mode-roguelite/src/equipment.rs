...
pub struct EquipmentConfig {
    ...
    /// Quel corps l'avatar 3ᵉ personne porte hors Expédition (le Hall). Le choix
    /// appartient à la donnée : c'est un réglage d'auteur, pas une règle moteur.
    /// Absent ou inconnu ⇒ `"base"`, et le moteur le dit.
    #[serde(default)]
    pub corps_troisieme_personne: Option<String>,
...
impl EquipmentConfig {
    ...
    /// Le corps par défaut : celui qui porte l'armure lootée. Toujours celui de
    /// l'aperçu du menu ; au Hall seulement si le génome le demande
    /// (`corps_troisieme_personne`, cf. `corps_hors_expedition`).
    ...
    /// Le corps de la 3ᵉ personne hors Expédition, tel que le génome le demande.
    ///
    /// Une valeur inconnue ne doit pas passer en silence : elle retombe sur le
    /// corps de base ET se signale. Un génome qui contient une faute de frappe
    /// donnerait sinon un Hall « inchangé » sans qu'on sache pourquoi.
    pub fn corps_hors_expedition(&self) -> CorpsAvatar {
        match self.corps_troisieme_personne.as_deref() {
            None | Some("base") => self.corps_de_base(),
            Some("expedition") => self.corps_expedition(),
            Some(autre) => {
                warn!(
                    "[equipment] corps_troisieme_personne = {autre:?} inconnu                      (attendu \"base\" ou \"expedition\") — repli sur le corps de base"
                );
                self.corps_de_base()
            }
        }
...
mod tests {
    ...
    /// Les captures Mixamo fusionnées dans `_fusil.glb` ont des orientations
    /// de rest-pose incompatibles avec le squelette natif. Tant qu'elles ne
    /// sont pas retargetées et validées sur mesh évalué, une simple édition du
    /// génome ne doit pas pouvoir les remettre en production par inadvertance.
    #[test]
    fn expedition_utilise_uniquement_les_clips_natifs_sans_deformation_connue() {
        let c = cfg();
        let a = &c.corps_expedition().anim;
        let natifs = [
            "death",
            "hammer_strike",
            "hit_react",
        ...
        // 🚨 L'EXCEPTION SE NOMME, elle ne s'obtient pas en élargissant la liste
        // des « natifs » (`controle-de-la-sortie` §5). Ces deux captures Mixamo
        // sont RÉACTIVÉES sciemment le 2026-08-21 : mappées sur `idle`, viser et
        // tirer ne jouaient rien du tout — un défaut vu à chaque tir, contre une
        // déformation qui n'a jamais été mesurée sur ces deux clips-là. Elles
        // repartent de cette liste dès que le retarget Blender est fait ; toute
        // AUTRE réactivation reste interdite par le contrôle ci-dessous.
        let exceptions_assumees = ["rifle_aim", "rifle_fire"];
        for (etat, clip) in [
            ("idle", &a.idle_clip),
            ("walk", &a.walk_clip),
            ("run", &a.run_clip),
            ("back", &a.back_clip),
            ("strafe", &a.strafe_clip),
            ("aim", &a.aim_clip),
            ("fire", &a.fire_clip),
            ("jump", &a.jump_clip),
            ("fall", &a.fall_clip),
            ("land", &a.land_clip),
            ("crouch_idle", &a.crouch_idle_clip),
            ("crouch_walk", &a.crouch_walk_clip),
            ("crouch_back", &a.crouch_back_clip),
            ("slide", &a.slide_clip),
            ("turn", &a.turn_clip),
        ] {
            assert!(
                natifs.contains(&clip.as_str()) || exceptions_assumees.contains(&clip.as_str()),
                "expedition/{etat} réactive le clip non retargeté « {clip} » — \
                 si c'est voulu, l'ajouter à `exceptions_assumees` AVEC sa raison"
            );
        }
    }
