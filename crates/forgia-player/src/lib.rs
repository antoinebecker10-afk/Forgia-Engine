...
pub struct PlayerLocomotion {
    ...
    /// Durée du dernier épisode hors-sol, conservée après le contact. Permet de
    /// distinguer un vrai atterrissage d'un papillonnement KCC sur une pente.
...
fn track_player_speed(
    ...
    let output = q_sol.single().ok();
    // Rapier applique le KCC après notre `FixedUpdate`. Quand une frame de
    // rendu rattrape plusieurs pas fixes, le Transform ne change donc qu'au
    // premier : les pas suivants mesuraient artificiellement zéro et faisaient
    // clignoter Marche→Idle→Marche pendant un déplacement continu. La sortie
    // KCC porte précisément la translation effectivement acceptée par la
    // physique ; le delta de Transform ne reste qu'un repli de bootstrap.
    let d = pas_effectif(
        output.map(|o| o.effective_translation),
        last.map(|lp| p - lp),
    );
    ...
    *dernier_lacet = lacet;
    ...
    let au_sol = output.map(|o| o.grounded).unwrap_or(true);
    ...
    if au_sol && !*au_sol_avant {
        loco.duree_vol_precedent_s = loco.depuis_decollage_s;
...
fn pas_effectif(sortie_kcc: Option<Vec3>, delta_transform: Option<Vec3>) -> Vec3 {
    sortie_kcc.or(delta_transform).unwrap_or(Vec3::ZERO)
}
...
mod tests {
    ...
    #[test]
    fn la_translation_effective_kcc_bat_le_delta_transform_fige() {
        let physique = Vec3::new(0.1, 0.0, 0.2);
        assert_eq!(
            pas_effectif(Some(physique), Some(Vec3::ZERO)),
            physique,
            "un pas fixe de rattrapage ne doit pas publier une fausse vitesse nulle"
        );
    }

    #[test]
    fn le_delta_transform_reste_le_repli_sans_sortie_kcc() {
        let delta = Vec3::new(0.0, -0.1, 0.3);
        assert_eq!(pas_effectif(None, Some(delta)), delta);
    }
