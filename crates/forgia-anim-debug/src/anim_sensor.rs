...
/// Cible volontairement pilotée après les clips (spring bones, IK spécialisé).
/// Le producteur externe pose ce marqueur pour que le capteur distingue une
/// cible abandonnée d'une cible animée par une autre couche.
#[derive(Component, Debug, Clone, Copy)]
pub struct ExternalAnimationTarget;
...
pub fn sys_write_anim_sensor(
    ...
    q_cibles: Query<&AnimationTargetId>,
    q_noms: Query<&Name>,
    q_external: Query<&AnimationTargetId, With<ExternalAnimationTarget>>,
) {
    ...
    *accum = 0.0;
    ...
    for (entite, nom, player, graphe_h, transitions) in q_lecteurs.iter().take(MAX_LECTEURS) {
        ...
        let external_ids: HashSet<AnimationTargetId> = q_external.iter().copied().collect();
        let mut secondary_names: Vec<String> = declarees
            .iter()
            .filter(|(id, _)| external_ids.contains(*id))
            .map(|(_, name)| name.clone())
            .collect();
        secondary_names.sort();
        let inertes = inertes(&declarees, &touchees)
            .into_iter()
...
fn fiche_json(f: &FicheLecteur) -> String {
    format!(
        r#"{{"nom":"{}","actifs":{},"poids_total":{:.3},"cibles_declarees":{},"cibles_touchees":{},"a_graphe":{},"a_transitions":{},"principal":{},"cibles_inertes":[{}],"cibles_mouvement_secondaire":[{}],"clips":[{}]}}"#,
        echappe(&f.nom),
        f.actifs,
        f.poids_total,
        f.cibles_declarees,
...
fn juger(fiches: &[FicheLecteur], total: usize) -> constat::Constat<constat::Pret> {
    ...
    if let Some(f) = fiches.iter().find(|f| {
        f.inertes
            .iter()
            .any(|name| name.starts_with("cheveux_") || name.starts_with("cloak_"))
    }) {
        return constat::alerte(format!(
            "ANIM_SECONDAIRE_ABSENTE : « {} » laisse des os attendus sans clip ni couche externe : {}",
            echappe(&f.nom),
            f.inertes
                .iter()
                .filter(|name| name.starts_with("cheveux_") || name.starts_with("cloak_"))
                .cloned()
                .collect::<Vec<_>>()
                .join(", ")
        ))
        .remede("brancher ces os au mouvement secondaire ou corriger les courbes du GLB");
    }
    // (b) — un lecteur qui ne joue rien. C'est la T-pose.
    if let Some(f) = fiches
        .iter()
        .find(|f| f.actifs == 0 && f.cibles_declarees > 0)
    {
        return constat::critique(format!(
    ...
    if let Some(f) = fiches.iter().find(|f| !f.a_graphe) {
        ...
        .remede(
            "poser un AnimationGraphHandle : en Bevy 0.18 un clip ne se joue QUE via un graphe",
        );
    }
    constat::ok()
}
...
mod tests {
    ...
    fn sans_nom_declare_on_rend_le_chemin_et_on_le_signale() {
        ...
        assert!(
            n.contains("sans nom declare"),
            "l'absence doit se voir : {n}"
        );
    }
    ...
    fn zero_lecteur_n_est_pas_un_feu_vert() {
        ...
        assert_eq!(
            ...
            "zero mesure doit etre info, jamais ok"
        );
        assert!(
    ...
    fn un_clip_qui_ne_touche_aucun_os_est_critique() {
        ...
        assert!(
            msg.contains("62"),
            "le message doit citer les os declares : {msg}"
        );
    }
    ...
    #[test]
    fn un_os_secondaire_sans_couche_externe_declenche_une_alerte() {
        let mut f = fiche("corps", 1, 1.0, 62, 58);
        f.inertes = vec!["cheveux_01".into(), "cloak_01".into()];
        let (s, msg) = juger(&[f], 1).verdict();
        assert_eq!(s, constat::Severite::Warn);
        assert!(msg.contains("ANIM_SECONDAIRE_ABSENTE"), "{msg}");
    }

    #[test]
    fn un_os_pris_par_le_mouvement_secondaire_n_est_pas_inerte() {
        let mut f = fiche("corps", 1, 1.0, 62, 58);
        f.secondaries = vec!["cheveux_01".into(), "cloak_01".into()];
        let (s, _) = juger(&[f], 1).verdict();
